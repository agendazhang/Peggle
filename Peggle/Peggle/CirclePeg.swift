...
/**
`CirclePeg` represents a circular peg in the game.

Since we need to encode the circular peg, it must conform to NSCoding and implement
the required functions.
*/
class CirclePeg: NSObject, NSCoding, PhysicsCircle {
    var uuid: UUID
    var x: CGFloat
    var y: CGFloat
    var color: PegColor
    ...
    // MARK: Equatable
    static func == (lhs: CirclePeg, rhs: CirclePeg) -> Bool {
        return lhs.x == rhs.x
            && lhs.y == rhs.y
            && lhs.color == rhs.color
    ...
    override var description: String {
        return "\(self.color) Circle Peg (\(self.x), \(self.y))"
    }
